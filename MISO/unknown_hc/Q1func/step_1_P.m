...
%         m_iter = m0;
        for j = 1:length(Q)
            ...
            error(j) = error_c + error_s - error_c .* error_s;error(error>0.3) = nan;
        ...
        error = error_c + error_s - error_c .* error_s;error(error>0.3) = nan;
        AoI = 0.5*m + m./(1-error);
    ...
    M11(i) = m_iter;
    QQ11(:,:,i) = Q_iter;   
    e_best(i) = error(find(AoI == min(AoI)));
end
save('Search_equal_P1.mat')
...
for i = 1:length(P)
    ...
    while 1
        ...
        for j = 1:length(Q)
            ...
            Pd = qfunc(w_s);
            error_s = 1 - Pd;
            z_th = real((sqrt(N+0.000001)*P_noise_c*Dc^2.5)/(Hc*Q(:,:,i)*Hc'))+0.0001;
            f = @(z_c) qfunc(sqrt(m_iter./(1-N./(real(Hc*Q(:,:,i)*Hc'./(P_noise_c*Dc^2.5))*z_c).^2)).*(log2(1+real(Hc*Q(:,:,i)*Hc'./(P_noise_c*Dc^2.5))*z_c)-d./m_iter)*log(2)).*chi2pdf(z_c,1);
    ...
    f_hat_x_hat_P_Dc1(i) = min(minAoI_iter);minAoI_iter = [];
    m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds^2.5));
...
%         m_iter = m0;
    ...
    f_hat_x_hat_Dc_d1(i) = min(minAoI_iter);minAoI_iter = [];
        m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds^2.5));
        w_s = (kappa - m_best(i).*real(trace(SNR_s1)))./(sqrt(2*m_best(i).*real(trace(SNR_s1))));     % 假设四根天线各自信道不相关,Pd只考虑四根sensing天线收到自己发送到target并反射回来的信号（i.e. 对角线上的元素）
        Pd = qfunc(w_s);
...
%         m_iter = m0;
    ...
    f_hat_x_hat_Dc_d2(i) = min(minAoI_iter);minAoI_iter = [];
        m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds^2.5));
        w_s = (kappa - m_best(i).*real(trace(SNR_s1)))./(sqrt(2*m_best(i).*real(trace(SNR_s1))));     % 假设四根天线各自信道不相关,Pd只考虑四根sensing天线收到自己发送到target并反射回来的信号（i.e. 对角线上的元素）
        Pd = qfunc(w_s);
...
%         m_iter = m0;
    ...
    f_hat_x_hat_Ds_d1(i) = min(minAoI_iter);minAoI_iter = [];
        m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds(i)^2.5));
        w_s = (kappa - m_best(i).*real(trace(SNR_s1)))./(sqrt(2*m_best(i).*real(trace(SNR_s1))));     % 假设四根天线各自信道不相关,Pd只考虑四根sensing天线收到自己发送到target并反射回来的信号（i.e. 对角线上的元素）
        Pd = qfunc(w_s);
...
%         m_iter = m0;
    ...
    f_hat_x_hat_Ds_d2(i) = min(minAoI_iter);minAoI_iter = [];
        m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds(i)^2.5));
        w_s = (kappa - m_best(i).*real(trace(SNR_s1)))./(sqrt(2*m_best(i).*real(trace(SNR_s1))));     % 假设四根天线各自信道不相关,Pd只考虑四根sensing天线收到自己发送到target并反射回来的信号（i.e. 对角线上的元素）
        Pd = qfunc(w_s);
...
%         m_iter = m0;
    ...
    f_hat_x_hat_d_Ds2(i) = min(minAoI_iter);minAoI_iter = [];
        m_best(i) = m_iter;
        SNR_s1 = trace(Hs*Q_iter*Hs'/(P_noise_s*Ds^2.5));
        w_s = (kappa - m_best(i).*real(trace(SNR_s1)))./(sqrt(2*m_best(i).*real(trace(SNR_s1))));     % 假设四根天线各自信道不相关,Pd只考虑四根sensing天线收到自己发送到target并反射回来的信号（i.e. 对角线上的元素）
        Pd = qfunc(w_s);

...
%         m_iter = m0;
        for j = 1:length(Q)
            ...
            f = @(z_c) qfunc(sqrt(m_iter./(1-(1./(1+Eigen(3)*real(Hc*Q(:,:,j)*Hc'./(P_noise_c*Dc(i)^2.5))*z_c./Nt).^2))).*(log2(1+real(Hc*Q(:,:,j)*Hc'./(P_noise_c*Dc(i)^2.5))*z_c)-d./m_iter)*log(2)).*chi2pdf(z_c,1);        
            error_c =  integral(@(z_c) f(z_c),0,Inf);
    
            error(j) = error_c + error_s - error_c .* error_s;error(error>0.1) = nan;
        ...
        error = error_c + error_s - error_c .* error_s;error(error>0.1) = nan;
        AoI = 0.5*m + m./(1-error);
    ...
    f_x_Dc_d2(i) = min(minAoI_iter); minAoI_iter = [];
    m_best(i) = m_iter;
    M21(i) = m_iter;
    QQ21(:,:,i) = Q_iter;       
    error(find(AoI == min(AoI)))
...
for i = 1:length(P)
    ...
    for j = 1:length(Q)
        ...
        Pd = marcumq(sqrt(2*m(j)*SNR_s1),sqrt(2*kappa),1);
        error_s = 1-Pd;
        error_c = 0.5;
        error = error_s+error_c-error_c*error_s;
        AoI(j) = 0.5*m(j) + m(j)/(1-error); 
    end
...
for i = 1:length(Ds)
    for j = 1:length(Q)
        SNR_c = real(Hc*Q(:,:,j)*Hc'/(P_noise_c*Dc^2.5));
        m = d/log2(1+SNR_c);
        SNR_s = real(trace(Hs*Q(:,:,j)*Hs'/(P_noise_s*Ds(i)^2.5)));
        Pd = marcumq(sqrt(2*m*SNR_s),sqrt(2*kappa),1);
        error_s = 1-Pd;
        error_c = 0.5;
        error = error_s+error_c-error_c*error_s;
        AoI(j) = 0.5*m + m/(1-error);         

...
w_s = (kappa - m_s.*SNR_s1)./(sqrt(2*m_s.*SNR_s1));     
Pd = qfunc(w_s);
...
run plot_setting.m
figure(2)   % error
surf(m_s,m_c,error);zlim([10^(-50) 0.5]);caxis([10^(-50) 0.5]);shading interp;xlabel('$m_s$');ylabel('$m_c$');zlabel('\epsilon');xlim([0 500]);ylim([0 500])
run plot_setting.m
% figure(3)   % error_c

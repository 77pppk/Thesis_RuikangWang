...
for i = 1:length(P)
    ...
    while 1
        ...
        error_s = 1 - qfunc((kappa-m_iter*SNR_s)./(sqrt(2*m_iter*SNR_s)));          
    ...
    error_c(error_c>0.5) = nan; 
        ...
        error_s = 1 - qfunc((kappa-m*SNR_s)./(sqrt(2*m*SNR_s))); % error_s(error_s>0.1) = nan;
        ...
        if k > 1 && abs(abs(minAoI_iter(k) - minAoI_iter(k-1))) < 0.1 
            break;
        end
        k = k + 1;        
    end

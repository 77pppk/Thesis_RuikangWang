...
run plot_setting.m;
figure(1)
surf(m_s,m_c,AoI_1);shading interp;xlabel('$m_s$');ylabel('$m_c$');zlabel('$\overline{\Delta}$', 'Interpreter', 'latex');
zlim([0 1500]);clim([0 1500])
run plot_setting.m;
figure(3)
...
% communication error
    ...
    run plot_setting.m;
figure(2)
surf(rho_s,m_s,AoI_1);shading interp;xlabel('$\rho_s$');ylabel('m');zlabel('$\overline{\Delta}$', 'Interpreter', 'latex');
zlim([0 1500]);clim([0 1500])
run plot_setting.m;
figure(4)
